...
export default function RoguelikePage() {
  ...
  const handleWordSolved = useCallback((word) => {
    ...
    // CHECK NESTED PERMANENTS:
    if (run?.permanents?.bonusTimeOnLong && word.answer.length >= 8) {
      addTime(30);
    }
  }, [addScore, addCoins, addTime, run?.permanents]);
  ...
  if (victory) return (
    <div className="min-h-screen bg-white text-black flex flex-col items-center justify-center gap-8 p-6">
      <div className="text-center max-w-sm">
        <h1 className="font-pixel text-black font-bold text-xl mb-4 leading-loose">You Won!</h1>
        <p className="text-gray-600 text-xs mb-2 leading-relaxed">You completed all {MAX_LEVEL} levels.</p>
        <div className="bg-gray-50 border-2 border-black rounded-xl p-4 mb-8 flex justify-around">
        ...
        <div className="flex flex-col gap-3">
          <button onClick={() => { resetRun(); navigate('/classic'); }} className="px-8 py-3 bg-black text-white font-pixel text-xs rounded-xl w-full">Play Classic Mode →</button>
          <button onClick={() => { resetRun(); navigate('/'); }} className="px-8 py-3 bg-white text-black border-2 border-black font-pixel text-xs rounded-xl w-full">Play Again</button>
        </div>
      </div>
    </div>
  );
  ...
  if (gameover) return (
    <div className="min-h-screen bg-white text-black flex flex-col items-center justify-center gap-6 p-6">
      <div className="text-center">
        <div className="text-5xl mb-4">💀</div>
        <p className="font-pixel text-black font-bold text-xl mb-2">Game Over</p>
        <p className="text-gray-500 text-xs">Final score: {run?.score ?? 0} pts · Level {run?.level ?? 1}</p>
      </div>
      <button onClick={() => { resetRun(); clearPuzzleState(); navigate('/'); }} className="px-6 py-3 bg-black text-white font-pixel text-xs rounded-xl w-full max-w-xs">Try Again</button>
    </div>
  );
  ...
  if (error) return (
    <div className="min-h-screen bg-white text-black flex flex-col items-center justify-center gap-4">
      ...
      <button onClick={() => navigate('/')} className="text-gray-600 text-xs underline">Home</button>
    </div>
  );
    ...
    <div className="min-h-screen bg-white text-black flex flex-col items-center p-4 gap-4">
      <div className="flex items-center justify-between w-full max-w-2xl pt-2">
        <div className="flex-1 flex items-center justify-start gap-3">
          <button onClick={handleGoHome} className="text-gray-400 hover:text-black text-xs font-pixel">← Home</button>
          <HeartsDisplay />
        </div>
        ...
        <div className="flex-1 flex items-center justify-end gap-3">
          <PuzzleTimer onTimeUp={handleTimeUp} paused={timerPausedRef.current || loading || levelComplete} />
          <button onClick={() => setShowShop(true)} className="text-black font-pixel text-xs underline decoration-2 underline-offset-4">Shop</button>
          <button onClick={handleQuitRun} className="text-red-400 hover:text-red-600 font-pixel text-base">↺</button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <>
          <CrosswordGrid
            puzzle={puzzle}
            revealedCells={new Set([...revealedCells, ...revealedVowels])}
          ...
          <div className="flex gap-3 flex-wrap justify-center pb-2">
            <button
              ...
              className="px-4 py-2 bg-white text-black border border-black hover:bg-gray-100 disabled:opacity-40 text-xs font-pixel rounded-lg"
            >
              Hint ({hintsRemaining})
            </button>
          </div>
          ...
          {levelComplete && (
            <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-40">
              <div className="bg-white border-4 border-black rounded-2xl p-10 text-center max-w-sm mx-4 shadow-[8px_8px_0px_rgba(0,0,0,1)]">
                ...
                <p className="text-black font-pixel text-xs mb-6">+{puzzle.words.reduce((s, w) => s + w.answer.length * 10, 0)} pts</p>
                <button onClick={handleNextLevel} className="px-8 py-3 bg-black text-white font-pixel text-xs rounded-xl w-full">
